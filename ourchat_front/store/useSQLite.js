...
function openSqlite(){
	...
	return new Promise((resolve,reject) =>{
		plus.sqlite.openDatabase({
			name:'hisMes',  //数据库名称
			path:'_doc/hisMes.db',   //数据库地址,uniapp推荐以下划线为开头
			success(e){
				resolve(e); //成功回调
			},
			fail(e){
...
//数据库不能存对象,数组
function userInfoSQL(userId){
	return new Promise((resolve,reject) =>{
	//创建表格在executeSql方法里写
		plus.sqlite.executeSql({
			name:'hisMes',
			//表格创建或者打开,后面为表格结构
			sql:'create table if not exists userInfo:"'+userId+'"("id" INTEGER PRIMARY KEY AUTOINCREMENT,"sender" INTEGER,"content" TEXT,"create_time" DATETIME)', 
			success(e){
				resolve(e);
			},
			fail(e){
...
//向表格里添加数据
//根据表格的列来添加信息
//因为id列我设为自动增加,所以不用添加数据
//values里是传过来要存的值,我这里是动态的,单引号加双引号拼接
function addUserInformation(obj,userId){
	//判断有没有传参
	if(obj !== undefined){
		//判断传的参是否有值
		...
		if(!b){
			//obj传来的参数对象
			var sender = obj.sender || null; //发送者
			var content = obj.content || null; //发送内容
			var creat_time = obj.creat_time || null; //发送时间
			return new Promise((resolve,reject) =>{
				plus.sqlite.executeSql({
					name:'hisMes',
					sql:'insert into userInfo:"'+userId+'"(sender,content,creat_time) values("'+sender+'","'+content+'","'+creat_time+'")',
					success(e){
						resolve(e);
					},
					fail(e){
