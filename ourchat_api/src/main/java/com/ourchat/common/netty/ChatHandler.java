...
public class ChatHandler extends SimpleChannelInboundHandler<TextWebSocketFrame> {
    ...
    private void handleWebSocketFrame(ChannelHandlerContext ctx, WebSocketFrame frame) {
        ...
        if (frame instanceof TextWebSocketFrame) {
            ...
            System.out.println("服务器收到信息"+requestmsg);
            String[] array= requestmsg.split(",");
            // 将通道加入通道管理器
            if(array.length==1){
                System.out.println("用户："+array[0]+"登录成功");
                UserInfoManager.addChannel(ctx.channel(),array[0]);
                //用户上线，发送收到的离线消息
                if(OfflineManagement.isExistsMessage(array[0])){
                    System.out.println("有历史消息");
                    List<OfflineMessage> list= OfflineManagement.readMessage(array[0]);
                    for(OfflineMessage offlineMessage:list){
                        UserInfoManager.broadcastMess(offlineMessage.getSender(),array[0],offlineMessage.getMessage());
                    }
                }
                ...
                UserInfoManager.broadcastMess(sendid,recevier,message);
            }
        }
    }
}
